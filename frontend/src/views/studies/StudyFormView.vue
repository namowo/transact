...
const [qualityCheckPassed] = defineField('quality_check_passed')

const { fields: authorFields, push: pushAuthor, remove: removeAuthorField } =
...
onMounted(async () => {
  ...
  try {
    ...
    const correspondingIndex = study.corresponding_author_name
      ? authors.findIndex(
          (author) => formatAuthorName(author) === study.corresponding_author_name,
        )
      : -1
    setValues({
      title: study.title ?? '',
...
const onSubmit = handleSubmit(async (values) => {
  ...
  try {
    const correspondingAuthor =
      values.correspondingAuthorIndex != null ? values.authors[values.correspondingAuthorIndex] : undefined
...
<template>
  <div class="flex flex-col gap-6 max-w-2xl">
    <h1 class="text-2xl font-bold text-surface-900 dark:text-surface-0">
      {{ editingId === null ? 'Add study' : 'Edit study' }}
    </h1>
    ...
    <form v-else class="flex flex-col gap-4" @submit.prevent="onSubmit">
      ...
      <div class="flex flex-col gap-2">
        <label for="study-journal" class="font-medium text-sm">Journal</label>
        <InputText id="study-journal" v-model="journal" v-bind="journalAttrs" fluid />
      </div>

      <div class="grid grid-cols-2 gap-4">
        <div class="flex flex-col gap-2">
          <label for="study-year" class="font-medium text-sm">Year</label>
          <InputNumber
            id="study-year"
            v-model="year"
            v-bind="yearAttrs"
        ...
        <div class="flex flex-col gap-2">
          <label for="study-doi" class="font-medium text-sm">DOI</label>
          <InputText id="study-doi" v-model="doi" v-bind="doiAttrs" fluid />
        </div>
      </div>

      <div class="flex flex-col gap-2">
        <label for="study-abstract" class="font-medium text-sm">Abstract</label>
        <Textarea id="study-abstract" v-model="abstractField" v-bind="abstractAttrs" rows="3" fluid />
      </div>

      <div class="flex flex-col gap-2">
        <label for="study-description" class="font-medium text-sm">Description</label>
        <Textarea
      ...
      <div class="flex flex-col gap-2">
        <label for="study-contact-author" class="font-medium text-sm"
          >Corresponding author contact</label
        >
        <Select
      ...
      <div class="grid grid-cols-2 gap-4">
        <div class="flex flex-col gap-2">
          <label for="study-contact-email" class="font-medium text-sm">Email</label>
          <InputText
        ...
        <div class="flex flex-col gap-2">
          <label for="study-contact-phone" class="font-medium text-sm">Phone number</label>
          <InputText
      ...
      <div class="flex flex-col gap-2">
        <span class="font-medium text-sm">Purpose *</span>
        <SelectButton
          v-model="purpose"
          ...
          option-value="value"
        />
        <Message v-if="errors.purpose" severity="error" size="small" variant="simple">
          {{ errors.purpose }}
        </Message>
      </div>

      <div class="flex items-center gap-3">
        <ToggleSwitch v-model="qualityCheckPassed" input-id="study-qc" />
        <label for="study-qc" class="text-sm">Quality check passed</label>
      </div>
